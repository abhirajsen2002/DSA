...
int isNonZero(sparseterm *s)
{ // checks if a matrix is null matrix
    return s[0].value > 0;
}

int compare(sparseterm A, sparseterm B, int col)
{ // compare positions of two terms to store in row major order
...
int inputMatrix(sparseterm *s)
{
    ...
    if (isNonZero(s))
    {
        ...
        for (int i = 1; i <= s[0].value; i++)
        {
            ...
            if ((s[i].row < 0) || (s[i].row >= s[0].row) || (s[i].col < 0) || (s[i].col >= s[0].col))
            {
...
void displayMatrix(sparseterm *s)
{
...
void transpose(sparseterm *A, sparseterm *B)
{
...
void storeSum(sparseterm *C, int *t, int r, int c, int v)
{ // attach a term to the resultant matrix
    if (v)
    { // if the value is non-zero

        if (*t < MAX_TERMS)
        { // if no of terms doesn't exceed MAX_TERMS
...
void add(sparseterm *A, sparseterm *B, sparseterm *C)
{
    if ((A[0].row == A[0].row) && (A[0].col == B[0].col))
    { // check compatibility for matrix addition
        ...
        while (i <= A[0].value && j <= B[0].value)
            ...
            case 0:
                storeSum(C, &totalC, A[i].row, A[i].col, A[i].value + B[j].value);
                i++;
                j++;
                break;
...
void mult(sparseterm *A, sparseterm *B, sparseterm *C)
{
    if (A[0].col != B[0].row)
    { // condition for incompatibility for matrix multiplication
        printf("Matrices incompatible for multiplication !\n");
        return;
    }

    sparseterm newB[MAX_TERMS];
    ...
    for (int i = 1; i <= A[0].value;)
    {
        ...
        for (int j = 1; j <= B[0].value + 1;)
        {
            ...
            {                                           // if no more non-zero term left in previous column of B
                storeSum(C, &totalC, row, column, sum); // store the sum as a new term in C
                sum = 0;
            ...
            { // compare terms in a particular row of A and column of B
                if (A[i].col < newB[j].col)
                    i++; // go to next term in A
                else if (A[i].col == newB[j].col)
                    sum += (A[i++].value * newB[j++].value); // add terms, go to next term in A and B
                else
