...
String input()
{
    ...
    for (int i = 0; i <= len; ++i)
    { // copy the input string into the null string
        S.s[i] = str[i];
    }
    return S;
}
...
int length(String S)
{
    int len = 0;
    while (S.s[len] != '\0')
    { // traverse till '\0' delimeter is reached
        ++len;
    }
    return len;
}
...
int compare(String S1, String S2)
{
    ...
    int i = 0;
    while (i < len1 && i < len2)
    {                           // check if current index is within the lengths of individual strings
